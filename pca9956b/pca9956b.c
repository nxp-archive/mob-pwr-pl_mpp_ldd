/*
 * nxp-lcdseg.c - NXP LCD segment driver
 *
...
static dev_t nxp_ledseg_dev_num;
 ...
 */
static int pca9956b_readWrite_reg(struct pca9956b_chip *chip, char *readStr, int writeRegAddr)
{
	...
	if (ret < 0) {
		dev_err(&chip->client->dev, "[%s]: Unable to read %s\n", __func__, readStr);
		return ret;
	}
	...
	if (ret < 0) {
		dev_err(&chip->client->dev, "[%s]: Unable to write %s , value = 0x%x\n", __func__, readStr, reg_value);
		return ret;
	}
		...
		const char *buf, size_t count)
{
    ...
    int ret, reg_value, reg_addr;

	ret = sscanf(buf, "%x %x", &reg_addr, &reg_value);
	if (ret == 0) {
		dev_err(&chip->client->dev, "[%s] fail to pca9956b out.\n", __func__);
		return count;
	}

    if(reg_addr < PCA9956B_MODE1 || reg_addr > PCA9956B_IREFALL){
		dev_err(&chip->client->dev, "[%s] Out of range. Reg = 0x%x \n", __func__,
		    reg_addr);
		return count;
    }
    ...
    if( ret != 0)
		dev_err(&chip->client->dev, "[%s] Operation [0x%x , %d] is failed.\n",
		...
		struct device_attribute *devattr, char *buf)
{
    ...
    for ( i = PCA9956B_MODE1 ; i < PCA9956B_IREFALL ; i++ )
    {
        ...
        if( ret != 0)
            dev_err(&chip->client->dev, "[%s] Reading reg[0x%x] is failed.\n", __func__, i);

        bufp += sprintf(bufp, "Addr[0x%x] = 0x%x\n", i, reg_value);
    }
		...
		struct device_attribute *devattr, char *buf)
{
    ...
    for ( i = PCA9956B_EFLAG0 ; i <= PCA9956B_EFLAG4 ; i++ )
    {
        ret = pca9956b_read_reg(chip, i, &reg_value);
        if( ret != 0)
            dev_err(&chip->client->dev, "[%s] Reading [0x%x] is failed.\n", __func__, i);

        bufp +=sprintf(bufp, "PCA9956B_EFLAG[%d] = 0x%x\n", i - PCA9956B_EFLAG0, reg_value);
    }
 ...
 */
static void pca9956b_brightness_set(struct led_classdev *led_cdev, enum led_brightness value)
{
	...
	mutex_lock(&chip->lock);
    ret = pca9956b_write_reg(chip, PCA9956B_PWM0 + pca9956b->led_num, value);
    if( ret != 0)
 ...
 */
static enum led_brightness pca9956b_brightness_get(struct led_classdev *led_cdev)
{
	struct pca9956b_led *pca9956b;
	struct pca9956b_chip *chip;
	int ret;
	...
	mutex_lock(&chip->lock);
    ret = pca9956b_read_reg(chip, PCA9956B_PWM0 + pca9956b->led_num, &reg_value);
    if( ret != 0)
		dev_err(&chip->client->dev, "[%s] is failed = %d.\n", __func__, ret);
...
static int pca9956b_setup(struct pca9956b_chip *chip)
{
	...
	ret = of_property_read_u32(np, "pca9956b,support_initialize", &reg_value);
	if (ret < 0) {
    ...
    ret = pca9956b_readWrite_reg(chip, "pca9956b,MODE1", PCA9956B_MODE1);
    if(ret < 0)
        return ret;

    ret = pca9956b_readWrite_reg(chip, "pca9956b,MODE2", PCA9956B_MODE2);
    if(ret < 0)
        return ret;

    ret = pca9956b_readWrite_reg(chip, "pca9956b,LEDOUT0", PCA9956B_LEDOUT0);
    if(ret < 0)
        return ret;

    ret = pca9956b_readWrite_reg(chip, "pca9956b,LEDOUT1", PCA9956B_LEDOUT1);
    if(ret < 0)
        return ret;

    ret = pca9956b_readWrite_reg(chip, "pca9956b,LEDOUT2", PCA9956B_LEDOUT2);
    if(ret < 0)
        return ret;

    ret = pca9956b_readWrite_reg(chip, "pca9956b,LEDOUT3", PCA9956B_LEDOUT3);
    if(ret < 0)
        return ret;

    ret = pca9956b_readWrite_reg(chip, "pca9956b,LEDOUT4", PCA9956B_LEDOUT4);
    if(ret < 0)
        return ret;

    ret = pca9956b_readWrite_reg(chip, "pca9956b,LEDOUT5", PCA9956B_LEDOUT5);
    if(ret < 0)
        return ret;
    ...
    {
        int reg_addr;
        ret = of_property_read_u32(np, "pca9956b,defaultIREF", &reg_value);
        if (ret < 0) {
            dev_err(&chip->client->dev, "[%s]: Unable to read pca9956b,defaultIREF\n", __func__);
            return ret;
        }
        mutex_lock(&chip->lock);
        ...
        for ( reg_addr = PCA9956B_IREF0 ; reg_addr <= PCA9956B_IREF23 ; reg_addr++ ){
            ret = pca9956b_write_reg(chip, reg_addr, (uint8_t)reg_value);
            if (ret < 0) {
                dev_err(&chip->client->dev, "[%s]: Unable to write reg0x%x[0x%x]\n", __func__, reg_addr, reg_value);
                mutex_unlock(&chip->lock);
                return ret;
            }
        }
					...
					const struct i2c_device_id *id)
{
    ...
    chip->leds = devm_kzalloc(&client->dev, sizeof(*led)*PCA9956B_LED_NUM, GFP_KERNEL);
    if(!chip->leds)
    {
    ...
    ret = alloc_chrdev_region(&nxp_ledseg_dev_num, 0, MAX_DEVICES, DRIVER_NAME);
    if (unlikely(ret)) {
        pr_err(DRIVER_NAME": failed to alloc chrdev region [%d]\n", ret);
        goto exit;
    }

    nxp_ledseg_class = class_create(THIS_MODULE, DRIVER_NAME);
    if(IS_ERR(nxp_ledseg_class)){
        printk(KERN_ALERT "Failed to register device class \n");
        ret = PTR_ERR(nxp_ledseg_class);
        goto err_class;
    }

	pca9956b_dev = device_create(nxp_ledseg_class, NULL, MKDEV(MAJOR(nxp_ledseg_dev_num), 0), chip, "pca9956b");
	if (IS_ERR(pca9956b_dev)) {
		dev_err(&client->dev,
...
err_sysfs:
    device_destroy(nxp_ledseg_class, MKDEV(MAJOR(nxp_ledseg_dev_num), 0));
err_device:
    class_unregister(nxp_ledseg_class);
    class_destroy(nxp_ledseg_class);
err_class:
    unregister_chrdev_region(nxp_ledseg_dev_num, MAX_DEVICES);
exit:
	devm_kfree(&client->dev, chip);
